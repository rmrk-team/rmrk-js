...
const runScript = async () => {
  ...
  for (const packagePath of packagePaths) {
    ...
    const file = await fs.promises.readFile(packagePath, 'utf-8');
    const packageJson = JSON.parse(file) as Package;
    ...
    let files: string[] = [];
    try {
      files = await fs.promises.readdir(dist);
    } catch {
    ...
    for (const [key, exports] of Object.entries(packageJson.exports)) {
      ...
      for (const [type, value] of Object.entries(exports) as [
        ...
        await fs.promises.symlink(srcFilePath, distFilePath, 'file');
      }
    }
  }
